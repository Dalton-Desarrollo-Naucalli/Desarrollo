...
Partial Class Funcionalidad_Agencias
...
#Region "Botones AGREGA,EDITA,CANCELA"
    ...
    Protected Sub cbEliminar_Callback(source As Object, e As CallbackEventArgs) Handles cbEliminar.Callback
        ...
        If glBusqueda.Value <> Nothing Then
            id = Convert.ToInt32(txtIdAgencia.Text)
        End If


        'Envía parámetros a SP "Spu_UtilDelete"

        Resultado = fnc.Fun_Elimina(15, id, 1, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
        If Resultado = "0" Then
...
"NULL"
        ...
        If (btnAgregar.Checked = False) And (btnEditar.Checked = True) Then
            id = Convert.ToInt32(txtIdAgencia.Text)
            Resultado = fnc.Fun_Edita(15, cadena, id, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
            R = Resultado.Split("|")
            If R(0) = "0" Then
        ...
        ElseIf (btnAgregar.Checked = True) And (btnEditar.Checked = False) Then
            Resultado = fnc.Fun_Nuevo(15, cadena, 2, dato, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
            R2 = Resultado.Split("|")
            If R2(0) = "0" Then
                cbActualizar.JSProperties("cpAccion") = "Registro agregado con éxito."
                cbActualizar.JSProperties("cpIdAgencia") = R2(1)
                idnew_Global = R2(1)
            Else
                cbActualizar.JSProperties("cpAccion") = Resultado
            End If
...
#End Region
    Private Sub LlenaCombos()
        ...
        'Combo proveedor
        DS = fncion.TraeDatosConsulta("Select 0 as ID, ' SELECCIONAR...' as NombreP union all Select prov_id as ID, nombre as NombreP from proveedor  order by NombreP", "ConexionDBS")
        'Combo 

        Me.cmbIdProveedor.DataSource = DS
        Me.cmbIdProveedor.ValueField = "ID"
        Me.cmbIdProveedor.TextField = "NombreP"
        Me.cmbIdProveedor.DataBind()
        Me.cmbIdProveedor.SelectedIndex = 0

    End Sub
    Public Sub LlenarBusqueda()
        'Este método llena el Gridlookup de Búsqueda mediante WS.
        glBusqueda.Columns.Clear()
        tabla = fnc.TraeDatosConsulta("SELECT * FROM SAC_agenciasTbl WHERE borradoLogico = 0", "ConexionDBS")
        ...
        If tabla.Tables.Count > 0 Then
            If tabla.Tables(0).Rows.Count > 0 Then
                ...
                For j As Integer = 0 To tabla.Tables(0).Columns.Count - 1
                    ...
                    If ColumnaGridt.Caption = "idAgencia" Then
                        ColumnaGridt.Caption = "ID Agencia"
