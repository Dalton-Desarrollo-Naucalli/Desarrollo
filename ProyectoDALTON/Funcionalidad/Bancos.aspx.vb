...
Partial Class Framework_Bancos
...
#Region "Botones AGREGA,EDITA,CANCELA"
    ...
    Protected Sub cbEliminar_Callback(source As Object, e As CallbackEventArgs) Handles cbEliminar.Callback
        ...
        Resultado = fnc.Fun_Elimina(36, id, 1, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
        If Resultado = "0" Then
    ...
    Private Sub CargaCallback()
        ...
        dato = Convert.ToString(txtNombreCorto.Text.Trim())
        Dim R() As String
        Dim R2() As String
...
"NULL"
        ...
        If (btnAgregar.Checked = False) And (btnEditar.Checked = True) Then
            id = Convert.ToInt32(txtIdBanco.Text)
            Resultado = fnc.Fun_Edita(36, cadena, id, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
            R = Resultado.Split("|")
            If R(0) = "0" Then
        ...
        ElseIf (btnAgregar.Checked = True) And (btnEditar.Checked = False) Then
            Resultado = fnc.Fun_Nuevo(36, cadena, 2, dato, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
            R2 = Resultado.Split("|")
            If R2(0) = "0" Then
...
#End Region
    Private Sub LlenaCombos()
        ...
        DS = fncion.TraeDatosConsulta("Select 0 as ID, ' SELECCIONAR...' as NombreF union all Select ID_formaliq as ID, Descripcion as NombreF from forma_liquidacion where escuenta=1 and Cheque_o_Caja= 'CO' and borradoLogico= 0 order by ID", "ConexionDBS")
        'Combo 
    ...
    Public Sub LlenarBusqueda()
        ...
        glBusqueda.Columns.Clear()
        tabla = fnc.TraeDatosConsulta("SELECT * FROM bancos WHERE borradoLogico = 0", "ConexionDBS")
        ...
        If tabla.Tables.Count > 0 Then
            If tabla.Tables(0).Rows.Count > 0 Then
                ...
                For j As Integer = 0 To tabla.Tables(0).Columns.Count - 1
                    ...
                    If ColumnaGridt.Caption = "idbanco" Then
                        ColumnaGridt.Caption = "Id Banco"
                        ColumnaGridt.ReadOnly = True
                        ColumnaGridt.Visible = True
                        ColumnaGridt.Width = 50
                    ElseIf ColumnaGridt.Caption = "nombre_corto" Then
                        ColumnaGridt.Caption = "Nombre corto"
                        ColumnaGridt.Visible = True
                    Else
