...
Partial Class Funcionalidad_MatricesContablesIII
...
#End Region
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            End If
            VerificaAccesos("MatricesContablesIII")
    ...
    Public Sub LlenarBusqueda()
        ...
        glBusqueda.Columns.Clear()
        tabla = fnc.TraeDatosConsulta("select * from matriz_contable_3 where borradoLogico = 0", "ConexionDBS")
    ...
    Private Sub LlenaCombos()
        ...
        DS = fncion.TraeDatosConsulta("SELECT DISTINCT id_conceptosIngresos,descripcion FROM conceptos_ingresos where borradoLogico = 0", "ConexionDBS")
        'Combo 
        Me.cmbConcepto.DataSource = DS
        Me.cmbConcepto.ValueField = "id_conceptosIngresos"
        Me.cmbConcepto.TextField = "descripcion"
        Me.cmbConcepto.DataBind()
        Me.cmbConcepto.SelectedIndex = 0

        Dim parametro As String = fnc.Parametro(46, "ConexionDBS")
        DS = fncion.TraeDatosConsulta("Select '  SELECCIONAR...' as ID, '  SELECCIONAR...' as NombreCI union all Select numerodecuenta as ID, descripcion as NombreCI from CatCon WHERE numerodecuenta Not Like '*000000' AND moneda_mex=" + parametro + " order by NombreCI", "ConexionGENDES")
        'Combo                         

        Me.cmbCuentaInterna.DataSource = DS
        Me.cmbCuentaInterna.ValueField = "ID"
        Me.cmbCuentaInterna.TextField = "ID"
        Me.cmbCuentaInterna.DataBind()
        Me.cmbCuentaInterna.SelectedIndex = 0

        Me.cmbCuentaInternaDes.DataSource = DS
        Me.cmbCuentaInternaDes.ValueField = "ID"
        Me.cmbCuentaInternaDes.TextField = "NombreCI"
        Me.cmbCuentaInternaDes.DataBind()
        Me.cmbCuentaInternaDes.SelectedIndex = 0


        DS = fncion.TraeDatosConsulta("Select '  SELECCIONAR...' as ID, '  SELECCIONAR...' as NombreCE union all Select numerodecuenta as ID, descripcion as NombreCE from CatCon WHERE numerodecuenta Not Like '*000000' AND moneda_mex= " + parametro + " order by NombreCE", "ConexionGENDES")
        'Combo 

        Me.cmbCuentaExterna.DataSource = DS
        Me.cmbCuentaExterna.ValueField = "ID"
        Me.cmbCuentaExterna.TextField = "ID"
        Me.cmbCuentaExterna.DataBind()
        Me.cmbCuentaExterna.SelectedIndex = 0

        Me.cmbCuentaExternaDes.DataSource = DS
        Me.cmbCuentaExternaDes.ValueField = "ID"
        Me.cmbCuentaExternaDes.TextField = "NombreCE"
        Me.cmbCuentaExternaDes.DataBind()
        Me.cmbCuentaExternaDes.SelectedIndex = 0
...
"NULL"
        ...
        If (btnAgregar.Checked = False) And (btnEditar.Checked = True) Then
            id = Convert.ToInt32(glBusqueda.Value)
            Resultado = fnc.Fun_Edita(67, cadena, id, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
            R = Resultado.Split("|")
            If R(0) = "0" Then
        ...
        ElseIf (btnAgregar.Checked = True) And (btnEditar.Checked = False) Then
            Resultado = fnc.Fun_Nuevo(67, cadena, 7, dato, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
            R2 = Resultado.Split("|")
            If R2(0) = "0" Then
                cbActualizar.JSProperties("cpAccion") = "Registro agregado con éxito."
                cbActualizar.JSProperties("cpIdBanco") = R2(1)
                idnew_Global = R2(1)
            Else
                cbActualizar.JSProperties("cpAccion") = Resultado
            End If
    ...
    Protected Sub cbEliminar_Callback(source As Object, e As CallbackEventArgs) Handles cbEliminar.Callback
        ...
        Resultado = fnc.Fun_Elimina(67, id, 1, Session("iIdUsuario"), "", 0, "", "ConexionDBS")
        If Resultado = "0" Then
    ...
    Public Sub ValidaControles()
        ...
        For Each Control In ASPxRoundPanel1.Controls
